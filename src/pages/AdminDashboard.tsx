...
const AdminDashboard = () => {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      const [profilesResult, documentsResult, admitCardsResult] = await Promise.all([
        supabase
          .from("profiles")
      ...
      const admitCardsData = admitCardsResult.data;

      // Map profiles, documents, and admit cards to applications
      const appsWithData = (appsData || []).map(app => ({
        ...
        admit_card: admitCardsData?.find(ac => ac.application_id === app.id) || null
      }));
      ...
      if (status === 'approved') {
        const { data: admitCard, error: admitError } = await supabase
          ...
          .insert([{
            ...
            admit_card_number: '' // Will be auto-generated by trigger
          }])
          .select()
          .single();

        if (admitError) {
          console.error("Error creating admit card:", admitError);
          toast({
            variant: "destructive",
            title: "Warning",
  ...
  const handleDownloadAdmitCard = (app: Application) => {
    ...
    generateAdmitCardPDF({
      ...
      applicationNumber: app.application_number,
      studentName: app.profiles?.full_name || "N/A",
      courseName: app.course_name,
      preferredCollege: app.preferred_college,
      stream: app.stream,
      generatedAt: app.admit_card.generated_at,
      studentEmail: app.profiles?.email || undefined,
      studentPhone: app.profiles?.phone || undefined,
    });
  };
